...
q_audits_percentile = 98 # Data points that have distances higher than q% of the rest are considered outliers
q_audits_std = 1.5
...
# id_audits = ds.index
id_audits = np.array([ds.index]).T


def get_anomalous_audits(ds, method='KMeans', reduce_dim=False, outlier_detection_method='Percentile'):
    '''
    Identifies anomalous audits using k-means clustering.
    ...
    outlier_detection_method : String, optional
        ...
        Sets the method to detect outliers. The default is 'Percentile'.
          - If it is set to 'Percentile', it assumes an outlier any sample that has a distance from the center of its corresponding cluster higher than q% of the other samples (i.e. audits).
          - If it is set to 'StandardDeviation', it assumes an outlier any sample that has a distance from the center q_audits_std times higher than the median distance + the standard deviation.
    ...
    '''
    print("Started scanning for anomalous audits.")
    if reduce_dim == True:
    ...
    if outlier_detection_method == 'Percentile':
        ...
        outlier_distance_0 = np.percentile(distances_cluster_0, q_audits_percentile)
        outlier_distance_1 = np.percentile(distances_cluster_1, q_audits_percentile)
        outlier_distance_2 = np.percentile(distances_cluster_2, q_audits_percentile)
        ...
        outliers_cluster_0 = ds_cluster_0[np.where(distances_cluster_0 > q_audits_std * (np.median(distances_cluster_0) + np.std(distances_cluster_0))), :][0,:,:]
        outliers_cluster_1 = ds_cluster_1[np.where(distances_cluster_1 > q_audits_std * (np.median(distances_cluster_1) + np.std(distances_cluster_1))), :][0,:,:]
        outliers_cluster_2 = ds_cluster_2[np.where(distances_cluster_2 > q_audits_std * (np.median(distances_cluster_2) + np.std(distances_cluster_2))), :][0,:,:]
    ...
    audits_prediction, number_of_outliers, outlier_percentage = utils.predict_batch(ds, outlier_detection_method, km, clusters_centers, distances_cluster_0, distances_cluster_1, distances_cluster_2, ds_cluster_0, ds_cluster_1, ds_cluster_2, q_audits_percentile, q_audits_std)
    print("K-Means found " + str(number_of_outliers) + " of the samples to be anomalous audits, out of the total of " + str(ds.shape[0]) + " audits.")
    print("Percentage of outliers in the data: " + str(round(outlier_percentage*100,2)) + "%")
        
    
    audits_prediction = pd.DataFrame(audits_prediction,
                                      index=id_audits[:,0],
                                      columns=['Prediction'])
    return audits_prediction


def get_anomalous_sensors(ds, audits_prediction, outlier_detection_method='Percentile'):
    '''
    ...
    audits_prediction : Pandas DataFrame
        The result of the audit anomaly detection. It's an array of size (M, 1).
    outlier_detection_method : String, optional
        Possible values: 'Percentile', 'StandardDeviation'
        Sets the method to detect outliers. The default is 'Percentile'.
          - If it is set to 'Percentile', it assumes an outlier any sensor that has a loss higher than mu_sensors_percentile% of the other sensors within the same audit.
          - If it is set to 'StandardDeviation', it assumes an outlier any sensor that has a loss mu_sensors_std times higher than the median error + the standard deviation of all sensor losses within the same audit.
    ...
    X_train = ds[audits_prediction.iloc[:,0] == 0].iloc[:,:48] # Train the model only on the non-anomalous samples
    X_validation = ds.iloc[0:1000].iloc[:,:48] # Validation set using the first 1000 samples (anomalous + non-anomalous)
    X_test = ds[audits_prediction.iloc[:,0] == 1].iloc[:,:48] # Test set is all the anomalous audits for which we want to identify faulty snesors
    ...
    sensors_prediction = pd.DataFrame(sensors_prediction, index=id_audits[audits_prediction==1], columns=sensor_names)
    ...
    sensors_prediction = pd.DataFrame(0,
                                  index=id_audits[audits_prediction==1],
                                  columns=sensor_names)
    if outlier_detection_method == 'Percentile':
        for id_audit in id_audits[audits_prediction==1]:
            for sensor_name in sensor_names:
                sensors_prediction.loc[id_audit, sensor_name] = np.where( abs(prediction_sensors_error.loc[id_audit, sensor_name]) > np.percentile(abs(prediction_sensors_error.loc[id_audit, :]), mu_sensors_percentile), 1, 0)
    else: # if outlier_detection_method == 'StandardDeviation'
        for id_audit in id_audits[audits_prediction==1]:
            for sensor_name in sensor_names:
                sensors_prediction.loc[id_audit, sensor_name] = np.where( abs(prediction_sensors_error.loc[id_audit, sensor_name]) > mu_sensors_std * (abs(np.median(prediction_sensors_error.loc[id_audit, :])) + abs(np.std(prediction_sensors_error.loc[id_audit, :]))), 1, 0)
    number_of_outliers = sensors_prediction.sum().sum()
    avg_number_of_outliers = round(number_of_outliers/X_test.shape[0],2)
    print("The autoencoder model found " + str(number_of_outliers) + " sensors to be anomalous, with an average of " + str(avg_number_of_outliers) + " faulty sensors per audit.")
    print("Percentage of faulty sensors per audit: " + str(round((avg_number_of_outliers/sensors_prediction.shape[1])*100,2)) + "%")
    return sensors_prediction
